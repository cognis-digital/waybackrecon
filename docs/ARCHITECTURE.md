...
- **rules/analyzers** apply the heuristics shipped in `waybackrecon/core.py`.
- **score** ranks by severity.
- **MCP server** (`waybackrecon mcp`) exposes `scan` for Cognis.Studio agents.
